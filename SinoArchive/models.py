...
def get_folder():
    ...
    if len(f) == 0 :
        ...
        newFolder.save()
        folder = os.path.join(settings.ARCHIVE_DIR,str(newFolder))
...
class ArchiveFile(models.Model):
    ...
    def get_backup_file(self):
        return os.path.join(settings.BACKUP_DIR, str(self.folder), f"{self.UUID}{self.ext}")
    def archiveThis(self):
        if not self.processed:
            final_folder = get_folder()
            if final_folder.totalSize + self.size > settings.ARCHIVE_SIZE:
                final_folder.full()
                final_folder = get_folder()
            self.folder = final_folder
            from_path = os.path.join(settings.MEDIA_DIR,self.file)
            if os.path.exists(from_path):
                to_folder = os.path.join(settings.ARCHIVE_DIR,str(final_folder))
