/**
 * WellDrawdown — 水理分析抽水預測系統 前端邏輯 (正算模式)
 */
(function () {
    'use strict';
    ...
    const DEFAULT_WELLS = [
        { name: 'W1', q: 2000, x: 70, y: 260 },
        { name: 'W2', q: 2000, x: 100, y: 260 },
        { name: 'W3', q: 2000, x: 140, y: 260 },
        { name: 'W4', q: 2000, x: 180, y: 260 },
        { name: 'W5', q: 2000, x: 220, y: 260 },
        { name: 'W6', q: 2000, x: 240, y: 280 },
        { name: 'W7', q: 2000, x: 240, y: 300 },
    ];
    ...
    function initWellTable(data) {
        ...
        tbody.innerHTML = '';
        data.forEach((w) => addWellRow(w.name, w.q, w.x, w.y));
    }

    function addWellRow(name, q = 2000, x = 0, y = 0) {
        const tbody = document.getElementById('well-tbody');
        ...
        tr.innerHTML = `
            ...
            <td><input type="number" value="${q}" step="10" class="well-q"></td>
                ...
                if (wIdx !== -1) {
                    ...
                    while ((m = arrRegex.exec(wText)) !== null) {
                        ...
                        if (vals.length >= 2 && !isNaN(vals[0]) && !isNaN(vals[1])) {
                            // 預設給定一個初估的 Q
                            wellPairs.push({ name: `W${wellPairs.length + 1}`, q: 2000, x: vals[0], y: vals[1] });
                        }
                    }
    ...
    function collectData() {
        ...
        wellRows.forEach(tr => {
            params.wells.push({
                ...
                q: parseFloat(tr.querySelector('.well-q').value) || 0,
