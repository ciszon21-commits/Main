...
def user_register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            login(request, user)
            messages.success(request, f'甇∟??????芾???典歇? {user.rpg_profile.character_class.name}')
            return redirect('engineer_rpg:dashboard')
    else:
...
def user_login(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user is not None:
                login(request, user)
                messages.success(request, f'甇∟???嚗user.username}嚗?)
                return redirect('engineer_rpg:dashboard')
            else:
                messages.error(request, '雿輻??蝔望?撖Ⅳ?航炊')
    else:
        form = UserLoginForm()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if not form.errors:
                ...
                messages.success(request, '?犖鞈??湔??嚗?)
                return redirect('engineer_rpg:dashboard')
    else:
        initial_data = {
...
def start_learning(request, skill_id):
    ...
    if user_skill.status in ['LOCKED', 'AVAILABLE']:
        ...
        user_skill.save()
        messages.success(request, f'??摮貊?嚗skill.name}')
...
def complete_skill(request, skill_id):
    ...
    if user_skill.status == 'IN_PROGRESS':
        ...
        while profile.experience >= profile.experience_to_next_level():
            ...
            profile.level += 1
            messages.success(request, f'?? ?剖???嚗??蝝?Lv.{profile.level}')
        
        profile.save()
        
        messages.success(request, f'摰???踝?{skill.name}嚗敺?{skill.exp_reward} 蝬??潘?')
...
def use_item(request, user_item_id):
    ...
    if user_item.quantity < 1:
        messages.error(request, '??賊?銝雲')
        return redirect('engineer_rpg:item_inventory')
    
    # ?桀???游?唬??葉雿輻嚗ㄐ?芣??隞
    # ?芯?憒????∪????瘀?憒?撽??嚗??臭誑?券ㄐ撖虫?
    
    messages.info(request, '甇日??瑕?賢??銝凋蝙??)
    return redirect('engineer_rpg:item_inventory')
...
def start_trial(request, trial_id):
    ...
    # ?箇? HP ?箏???3嚗????憭?蝞?
    base_hp = 3
    total_hp = profile.get_total_hp() if hasattr(profile, 'get_total_hp') else 3
    initial_mp = 100
...
def start_daily_trial(request, task_id):
    ...
    if progress.is_completed:
        messages.warning(request, '甇支遙?歇摰?')
        return redirect('engineer_rpg:daily_trial_list')
    
    # 憒? HP 甇賊嚗??賜匱蝥?
    if progress.current_hp <= 0:
        messages.error(request, '??澆歇甇賊嚗遙?仃??)
        return redirect('engineer_rpg:daily_trial_list')
    ...
    # 瑼Ｘ?臬????
    if not questions:
        messages.error(request, '甇支遙??憿澈憿銝雲?瘜脰?嚗??舐窗蝞∠??⊥憓???)
        return redirect('engineer_rpg:daily_trial_list')
...
def submit_trial(request, trial_id):
    ...
    if is_passed:
        ...
        while profile.experience >= profile.experience_to_next_level():
            ...
            # 撅祆扳???蝷?
            msg_parts = [f'?? ?剖???嚗??蝝?Lv.{profile.level}']
            ...
            messages.success(request, '嚗?.join(msg_parts) + '嚗?)
        ...
        if not is_dungeon_repeat:
            ...
            if trial.item_reward:
                # 瑼Ｘ?拙振?臬撌脩???嚗?隤芣瘨?嚗? UserItem 閮?摨怠?嚗?
                ...
                record.save()
                messages.success(request, f'?脣??嚗trial.item_reward.name}')
            ...
            messages.info(request, '?????唬????⊥??脣??寞??嚗?撽澆之撟?撠?)
...
def apply_promotion(request):
    ...
    if pending_request:
        messages.warning(request, '?典歇??撖拇???隢?')
        return redirect('engineer_rpg:dashboard')
    ...
    if completed_skills < required_skills.count():
        messages.error(request, '撠摰????靽格??踝?')
        return redirect('engineer_rpg:skill_tree')
    ...
    messages.success(request, f'撌脫?鈭斗??隢?Lv.{profile.level} ??Lv.{target_level}')
    return redirect('engineer_rpg:dashboard')
...
def manager_dashboard(request):
    ...
    if profile.role not in ['MANAGER', 'ADMIN']:
        messages.error(request, '?冽????赤?迨?嚗?)
        return redirect('engineer_rpg:dashboard')
...
def approve_request(request, request_id):
    ...
    if profile.role not in ['MANAGER', 'ADMIN']:
        messages.error(request, '?冽????銵迨??嚗?)
        return redirect('engineer_rpg:dashboard')
    ...
    if promotion_request.status == 'PENDING':
        ...
        messages.success(request, f'撌脤? {applicant.user.username} ???隢?')
...
def reject_request(request, request_id):
    ...
    if promotion_request.status == 'PENDING':
        ...
        messages.success(request, f'撌脫?蝯?{promotion_request.applicant.user.username} ???隢?')
...
def create_question(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            messages.success(request, '憿撱箇???嚗?)
            return redirect('engineer_rpg:question_management')
        except Exception as e:
            messages.error(request, f'撱箇?憭望?嚗str(e)}')
...
def edit_question(request, question_id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            question.save()
            messages.success(request, '憿?湔??嚗?)
            return redirect('engineer_rpg:question_management')
        except Exception as e:
            messages.error(request, f'?湔憭望?嚗str(e)}')
...
def edit_dungeon(request, dungeon_id):
    ...
    if request.method == 'POST':
        ...
        if is_locked:
            # 憒?撌脤?摰??芾靽格????
            dungeon.save()
            messages.warning(request, '甇文銝?撌脫???閮?嚗??湔????)
            return redirect('engineer_rpg:dungeon_management')
        ...
        if 'parents' in data:
            ...
            if parent_ids is not None:
                ...
                for parent in parent_skills:
                    if node_type == 'ROOT':
                        ...
                        if parent.node_type != 'ROOT':
                            return JsonResponse({
                                'error': f'?勗?敹耨??賢?賡?隞??靽格??賭??箏?蝵柴?{parent.name}" ??{parent.get_node_type_display()}??
                            }, status=400)
                    elif node_type == 'CORE':
                        # CORE can only have ROOT or CORE parents
                        if parent.node_type not in ['ROOT', 'CORE']:
                            return JsonResponse({
                                'error': f'?瑟平?詨???賢?賡???靽格??瑟平?詨?雿?蔭??{parent.name}" ??{parent.get_node_type_display()}??
                            }, status=400)
                ...
                def would_create_cycle(node_id, new_parent_ids):
                    """
                    瑼Ｘ撠?new_parent_ids 閮剔 node_id ??蝵格??賣?行???敺芰靘陷
                    雿輻瘛勗漲?芸??揣 (DFS) 瑼Ｘ葫敺芰
                    """
                    # 撱箇??嗅???鞈游?嚗???喳?靽格??暺?
                ...
                if would_create_cycle(node.id, parent_ids):
                    return JsonResponse({
                        'error': '瑼Ｘ葫?啣儐?唬?鞈湛?閮剖????蔭??賣?敶Ｘ?敺芰??嚗?瑼Ｘ??賣邦蝯???
                    }, status=400)
...
def api_auto_layout_skill_tree(request):
    ...
    try:
        ...
        return JsonResponse({
            ...
            'count': updated_count,
            'levels': len(levels),
            'message': f'撌脰??撅 {updated_count} ???賜?暺???{len(levels)} 撅?
        })
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                if result['success'] > 0:
                    messages.success(request, f'???臬 {result["success"]} 蝑??殷?')
                if result['skip'] > 0:
                    messages.warning(request, f'頝喲? {result["skip"]} 蝑???)
                if result['errors']:
                    for error in result['errors'][:5]:
                        messages.error(request, error)
                return redirect('engineer_rpg:question_management')
            except Exception as e:
                messages.error(request, f'?臬憭望?嚗str(e)}')
    else:
...
def guild_post_create(request):
    ...
    if request.method == 'POST':
        ...
        if category == 'ANNOUNCEMENT' and profile.role not in ['OFFICER', 'MANAGER', 'ADMIN']:
            messages.error(request, '?芣?蝞∠??∪隞亦撣??)
...
def team_member_detail(request, team_id, member_id):
    ...
    if not (is_team_leader or is_manager):
        messages.error(request, '?冽?????迨鞈?')
        return redirect('engineer_rpg:index')
    
    # 蝣箄?閰脫??∪惇?潭迨??
    if member.current_team != team:
        messages.error(request, '閰脫??∩?撅祆甇日?隡?)
...
def team_add_member(request, team_id):
    ...
    # 瑼Ｘ甈?
    if team.leader != request.user:
        messages.error(request, '?芣???臭誑?啣??')
        return redirect('engineer_rpg:team_detail', team_id=team_id)
    
    member_id = request.POST.get('member_id')
    if not member_id:
        messages.error(request, '隢???啣?????)
        return redirect('engineer_rpg:team_manage_members', team_id=team_id)
    ...
    try:
        ...
        if member_profile.current_team:
            messages.error(request, f'{member_profile.user.username} 撌脩??典隞?隡葉')
            return redirect('engineer_rpg:team_manage_members', team_id=team_id)
        ...
        messages.success(request, f'??撠?{member_profile.user.username} ???')
        
    except UserProfile.DoesNotExist:
        messages.error(request, '?曆??啗府雿輻??)
...
def team_remove_member(request, team_id, member_id):
    ...
    try:
        ...
        if member_profile.current_team != team:
            messages.error(request, '閰脖蝙?刻??冽迨??銝?)
            return redirect('engineer_rpg:team_manage_members', team_id=team_id)
        ...
        messages.success(request, f'??撠?{member_profile.user.username} 蝘餃??')
...
def _seed_default_items():
    ...
    items_data = [
        # Basic
        {'name': '撌亦?靘踵?', 'item_type': 'BASIC', 'effect_type': 'HINT', 'effect_value': 0, 'rarity': 'COMMON', 'desc': '憿舐內?嗅?憿?????萄??內??},
        {'name': '???', 'item_type': 'BASIC', 'effect_type': 'HEAL', 'effect_value': 10, 'rarity': 'COMMON', 'desc': '蝡?儔 10% HP??},
        {'name': '鈭銝蝖砍馳', 'item_type': 'BASIC', 'effect_type': 'REMOVE_OPTION', 'effect_value': 1, 'rarity': 'COMMON', 'desc': '?冽?蝘駁銝?隤日??},
        # Advanced
        {'name': '摰?脰風蝬?, 'item_type': 'ADVANCED', 'effect_type': 'SHIELD', 'effect_value': 1, 'rarity': 'RARE', 'desc': '?菜?銝?甈∠??舫??摰喋?},
        {'name': '撌交?撅辣隞?, 'item_type': 'ADVANCED', 'effect_type': 'TIME_EXTEND', 'effect_value': 30, 'rarity': 'RARE', 'desc': '撱園?祇??∪?? 30 蝘?},
        {'name': '蝎曉?皜祇??', 'item_type': 'ADVANCED', 'effect_type': 'DISTRIBUTION', 'effect_value': 0, 'rarity': 'RARE', 'desc': '憿舐內甇?Ⅱ蝑?????雿?},
        # Rare
        {'name': '?芸????, 'item_type': 'RARE', 'effect_type': 'SKIP', 'effect_value': 0, 'rarity': 'EPIC', 'desc': '?湔頝喲??嗅?憿??},
        {'name': '蝺亦雁靽桀?', 'item_type': 'RARE', 'effect_type': 'HEAL', 'effect_value': 50, 'rarity': 'EPIC', 'desc': '蝡?儔 50% HP??},
        {'name': '蝯?瘜???, 'item_type': 'RARE', 'effect_type': 'COMBO_PROTECT', 'effect_value': 1, 'rarity': 'LEGENDARY', 'desc': '??蝑銋??葉?琿??蝝??},
    ]
...
def api_consume_item(request, user_item_id):
    ...
    try:
        ...
        if user_item.quantity < 1:
            return JsonResponse({'success': False, 'error': '??賊?銝雲'})
        ...
        return JsonResponse({'success': False, 'error': '?銝???}, status=404)
    except Exception as e:
