...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            ...
            civil_1_2 = SkillNode.objects.create(
                name='模板工程支撐計算',
                description='掌握模板支撐系統的設計與計算，確保施工安全。',
                node_type='CORE',
                character_class=civil,
                exp_reward=100,
                position_x=1,
            ...
            civil_1_2.parent_skills.add(novice_1, novice_2)
            ...
            civil_1_key.parent_skills.add(civil_1_1, civil_1_2)
            ...
            root_skills = SkillNode.objects.filter(node_type='ROOT').count()
            core_skills = SkillNode.objects.filter(node_type='CORE').count()
            advanced_skills = SkillNode.objects.filter(node_type='ADVANCED').count()
            
            self.stdout.write(self.style.SUCCESS('\n' + '=' * 60))
            self.stdout.write(self.style.SUCCESS('✅ 完整技能樹資料初始化完成！'))
            self.stdout.write(self.style.SUCCESS('=' * 60))
            self.stdout.write(f'   📊 技能總數：{total_skills} 個')
            self.stdout.write(f'   🌱 共同必修 (ROOT)：{root_skills} 個')
            self.stdout.write(f'   🌳 職業核心 (CORE)：{core_skills} 個')
            self.stdout.write(f'   🍃 進階選修 (ADVANCED)：{advanced_skills} 個')
            self.stdout.write(f'\n   ⚔️  土木戰士：{SkillNode.objects.filter(character_class__code="CIVIL").count()} 個技能')
            self.stdout.write(f'   🔮 機電法師：{SkillNode.objects.filter(character_class__code="ME").count()} 個技能')
            self.stdout.write(f'   🛡️  職安僧侶：{SkillNode.objects.filter(character_class__code="SAFETY").count()} 個技能')
            self.stdout.write(self.style.SUCCESS('=' * 60))
